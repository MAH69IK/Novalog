...
static int doLog(const char * fmt, ...)
{
    ...
    return processLogLine(LOG_SYSLOG, "novalog", infobuf);
}
...
static void novalog_signal_exit(int exit_status)
{
    exit_hook();
    _exit(exit_status);
}
...
static RETSIGTYPE sigkchld(int sig)
{
    ...
    novalog_signal_exit(EXIT_FAILURE);
}
